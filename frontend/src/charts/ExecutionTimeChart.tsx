    ...
    <div className="bg-slate-900 border border-slate-700 rounded-lg p-3 shadow-xl">
      ...
      {payload.map((p: any) => (
        <div key={p.dataKey} className="flex items-center gap-2 text-xs">
          ...
          <span className="font-mono text-slate-200">{p.value?.toFixed(4)}s</span>
        </div>
      ))}
    </div>
  )
...
export default function ExecutionTimeChart({ data }: Props) {
  ...
  const chartData = workerCounts.map((wc) => {
    ...
    for (const w of workloads) {
      ...
      entry[w] = point?.execution_time ?? null
    }
    return entry
  })
  ...
  if (data.length === 0) {
    return (
      <div className="card p-5 flex flex-col gap-2">
        <h3 className="text-slate-100 text-sm font-semibold">Execution Time</h3>
  ...
  return (
    <div className="card p-5">
      <div className="mb-4">
        <h3 className="text-slate-100 text-sm font-semibold">Execution Time vs Worker Count</h3>
        <p className="text-xs text-slate-500 mt-0.5">Lower is better – parallel wall-clock time (seconds)</p>
      </div>
      <ResponsiveContainer width="100%" height={240}>
        <BarChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 4 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
          <XAxis dataKey="workers" stroke="#334155" tickFormatter={(v) => `${v}P`} />
          <YAxis stroke="#334155" tickFormatter={(v) => `${v}s`} />
